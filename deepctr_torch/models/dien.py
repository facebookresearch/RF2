...
class InterestExtractor(nn.Module):
    ...
    def forward(self, keys, keys_length, neg_keys=None):
        ...
        # KIWAN: Added the second return to be simply all zero.
        if masked_keys_length.shape[0] == 0:
            return zero_outputs, torch.zeros((1,), device=keys.device)

        masked_keys = torch.masked_select(keys, mask.view(-1, 1, 1)).view(-1, max_length, dim)

        # KIWAN: Added .cpu() to length (related: https://github.com/pytorch/pytorch/issues/43227)
