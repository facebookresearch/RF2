...
from fl_libs.init import init_mlp
...
class DIN(BaseModel):
    ...
    def __init__(self, dnn_feature_columns, history_feature_list, dnn_use_bn=False,
                 ...
                 seed=1024, task='binary', device='cpu', gpus=None, dice_norm_type='bn', dice_gn_group=2, channel_multiplier=1.0, args=None):
        ...
        if len(self.history_feature_columns) > 0:
            self.attention = AttentionSequencePoolingLayer(att_hidden_units=att_hidden_size,
                                                           embedding_dim=att_emb_dim,
                                                           att_activation=att_activation,
                                                           return_score=False,
                                                           supports_masking=False,
                                                           dice_norm_type=dice_norm_type,
                                                           dice_gn_group=dice_gn_group,
                                                           weight_normalization=att_weight_normalization, channel_multiplier=channel_multiplier, args=args)
        ...
        self.dnn = DNN(inputs_dim=self.compute_input_dim(dnn_feature_columns),
                       ...
                       l2_reg=l2_reg_dnn,
                       device=device,
                       use_bn=dnn_use_bn, channel_multiplier=channel_multiplier)
        self.dnn_linear = nn.Linear(dnn_hidden_units[-1], 1, bias=False).to(device)
        init_mlp(self.dnn_linear)
        self.to(device)
    ...
    def forward(self, X):
        ...
        return y_pred, emb_time, mlp_time
    ...
    def set_channel_multiplier(self, channel_multiplier):
        self.dnn.channel_multiplier = channel_multiplier
        self.attention.local_att.dnn.channel_multiplier = channel_multiplier
