...
class AttentionSequencePoolingLayer(nn.Module):
    ...
    def __init__(self, att_hidden_units=(80, 40), att_activation='sigmoid', weight_normalization=False,
                 return_score=False, supports_masking=False, embedding_dim=4, dice_norm_type='bn', dice_gn_group=2, channel_multiplier=1.0, args=None, **kwargs):
        ...
        self.local_att = LocalActivationUnit(hidden_units=att_hidden_units, embedding_dim=embedding_dim,
                                             ...
                                             dropout_rate=0, use_bn=False, dice_norm_type=dice_norm_type, dice_gn_group=dice_gn_group, channel_multiplier=channel_multiplier)

        self.args = args # TODO: Only for testing
    ...
    def forward(self, query, keys, keys_length, mask=None):
        ...
        if self.supports_masking:
            ...
            #print("Masks: ", keys_masks, keys_masks.shape)

        attention_score = self.local_att(query, keys)  # [B, T, 1]
        #print(query, keys, keys_masks)
        ...
        # KIWAN TODO: Remove the attention and do uniform pooling
        if self.args is None or self.args.attention == "":
            outputs = torch.where(keys_masks, outputs, paddings)  # [B, 1, T]
            if self.weight_normalization:
                outputs = F.softmax(outputs, dim=-1)  # [B, 1, T]
        elif self.args.attention == "uniform":
            ones = torch.ones_like(outputs)
            outputs = torch.where(keys_masks, ones, paddings)  # [B, 1, T]
            #print(outputs, outputs.shape)
            if self.weight_normalization:
                outputs = torch.where((outputs < -2 * 32 + 2).bool(), torch.zeros_like(outputs), F.softmax(outputs, dim=-1))
        elif self.args.attention == "corrected":
            outputs = torch.where(keys_masks, outputs, paddings)  # [B, 1, T]
            if self.weight_normalization:
                outputs = torch.where((outputs < -2 * 32 + 2).bool(), torch.zeros_like(outputs), F.softmax(outputs, dim=-1))
        else:
